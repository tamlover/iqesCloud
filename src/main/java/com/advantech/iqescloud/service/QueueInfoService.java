...
public class QueueInfoService {
    ...
    public List<ChurnRateDTO> getChurnRateDTO(Long restaurantId,String date) throws ParseException {
        ...
        for (String day:week){
            ...
            churnRateDTO.setDate(day);

            /**
             * get churnRate
             */
            int all=queueInfoDao.getByQueueStartTime(day+"%",restaurantId);
            int churn=queueInfoDao.getByQueueStartTimeAndQueueState(day+"%",restaurantId);
            float rate= Float.parseFloat(decimalFormat.format((float)churn/all));

            churnRateDTO.setRate(rate);
